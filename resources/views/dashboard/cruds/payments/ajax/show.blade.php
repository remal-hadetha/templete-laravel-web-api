...
<div class="bg-image" style="background-image: url('{{asset("media/photos/photo8@2x.jpg")}}');">
    <div class="bg-black-50">
        <div class="content content-full text-center">
            <div class="my-3">
                @if ($data->order->services->first())
                <img class="img-avatar img-avatar-thumb" src="{{$data->order->services->first()->ImageURL}}" alt="">

                @endif
            </div>
            <h1 class="h2 text-white mb-0"> الإجمالي {{$data->order->sub_total}}</h1>
    ...
    <div class="content content-boxed">
        <div class="row items-push text-center">
            <div class="col-6 col-md-3">
                ...
                <a class="link-fx font-size-h3" href="javascript:void(0)">{{$data->order->user->name}}</a>
            </div>
            <div class="col-6 col-md-3">
                <div class="font-size-sm font-w600 text-muted text-uppercase"> إسم صاحب الصالون </div>

                <a class="link-fx font-size-h3" href="javascript:void(0)"> {{$data->order->salon->name}}</a>
            </div>

            <div class="col-6 col-md-3">
                <div class="font-size-sm font-w600 text-muted text-uppercase">القسم</div>
                <a class="link-fx font-size-h3" href="javascript:void(0)">@if($data->order->salon->category_id != null) {{$data->order->salon->category->name_ar}} @endif</a>
            </div>
                    ...
                    <div class="timeline-event-block block invisible" data-toggle="appear">
                        ...
                        <div class="block-content block-content-full">
                            <p class="font-w600 mb-2">
                                تاريخ تنفيذ الطلب
                                {{date('Y-m-d',strtotime($data->order->date)) }}

                            </p>
                            <div class="d-flex">
                                @if(count($data->order->services) > 0)
                                @forelse ($data->order->services as $item)
                ...
                <li class="timeline-event">
                    ...
                    <div class="timeline-event-block block invisible" data-toggle="appear">
                        ...
                        <div class="block-content">
                           ...
                           <div class="d-flex">
                            @if(count($data->order->salon->works) > 0)
                            @forelse ($data->order->salon->works as $item)
                            <a class="item item-rounded bg-info mr-2" href="{{route('services.show',$item->id)}}">
                                <img src="{{$item->ImageURL}}" style="width: 65px;
                                height: 65px;" alt="">
                            </a>
                ...
                <div class="block-content">
                        <div class="media d-flex align-items-center push">
                            ...
                            <div class="media-body">
                                <div class="font-w800">  {{$data->order->payment_type}} </div>
                            </div>
                        </div>
